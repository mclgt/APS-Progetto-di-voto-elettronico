...
class MerkleTree:
    def __init__ (self, schede_validate: List[Dict[str, Any]]):
        ...
        leaves = [self.hash_leaf(s["encrypted_vote"], s["token_voto"]) for s in self.schede]

        if leaves:
            self._build_tree(leaves)

    @staticmethod
    def hash_leaf(encrypted_vote: dict, token_voto: str)->bytes:
        ...
        payload = enc_vote_bytes+b"||"+token_voto.encode("utf-8")
